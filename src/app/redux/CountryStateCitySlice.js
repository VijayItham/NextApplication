...
export const fetchCountry = createAsyncThunk("fetchCountry", async () => {
  const response = await axios.get(
    "https://devrechargeapi.codetrex.in/api/CountryStateCity/getAllCountry"
  );
...
export const fetchState = createAsyncThunk("fetchState", async (id) => {
  const response = await axios.get(
    `https://devrechargeapi.codetrex.in/api/CountryStateCity/getAllState/` + id
  );
...
export const fetchCity = createAsyncThunk("fetchCity", async (id) => {
  const response = await axios.get(
    `https://devrechargeapi.codetrex.in/api/CountryStateCity/getAllCity/` + id
  );
  return response.data;
});
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCountry.fulfilled, (state, action) => {
        (state.isLoading = false), (state.countryList = action.payload);
      })
      ...
      .addCase(fetchState.fulfilled, (state, action) => {
        if (action.payload.statusCode == 200) {
          (state.isLoading = false), (state.stateList = action.payload.data);
        }
      })
      ...
      .addCase(fetchCity.fulfilled, (state, action) => {
        if (action.payload.statusCode == 200) {
          (state.isLoading = false), (state.cityList = action.payload.data);
        }
      })
